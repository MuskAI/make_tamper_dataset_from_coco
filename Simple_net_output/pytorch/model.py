...
def side_branch(x, input_channel, output_channel, factor):
    x = nn.Conv2d(input_channel, output_channel, kernel_size=1)

    kernel_size = (2 * factor, 2 * factor)
    x = nn.ConvTranspose2d(input_channel,output_channel,kernel_size=kernel_size,padding_mode='replicate',bias=False)
    return x


class aspp(nn.Sequential):
    def __init__(self, x, input_shape, out_stride):
        super(aspp, self).__init__()
        self.b0 = nn.Sequential(OrderedDict([
            ('b0_conv', nn.Conv2d(128, 128, kernel_size=1, padding_mode='replicate', bias=False)),
        ...
        self.b1 = nn.Sequential(OrderedDict([
            ('b1_depthwise_conv',
             nn.Conv2d(128, 128, kernel_size=3, groups=128, dilation=(6, 6), padding_mode='replicate', padding=1,
                       bias=False)),
        ...
        self.b2 = nn.Sequential(OrderedDict([
            ('b2_depthwise_conv',
             nn.Conv2d(128, 128, kernel_size=3, groups=128, dilation=(12, 12), padding_mode='replicate', padding=1,
                       bias=False)),
        ...
        self.b4 = nn.Sequential(OrderedDict([
            ...
            ('b4_bilinearUpsampling', nn.UpsamplingBilinear2d(self.out_shape, self.out_shape1))
        ]))
    ...
    def forward(self, x):
        ...
        b3 = self.b3(b2)
        b4 = self.b4(b3)
        x = torch.cat([b4, b0, b1, b2, b3], dim=3)
        return x
...
class Model(nn.Module):
    def __init__(self, input_shape=(320, 320, 3)):
        ...
        self.block1_maxpool = nn.MaxPool2d(kernel_size=2, stride=2, padding=1)  # 160 160 64
        ...
        self.block2_maxpool = nn.MaxPool2d(kernel_size=2, stride=2, padding=1)  # 80 80 128
        ...
        self.block3_maxpool = nn.MaxPool2d(kernel_size=2, stride=2, padding=1)  # 80 80 256
        ...
        self.block4_maxpool = nn.MaxPool2d(kernel_size=2, stride=2, padding=1)  # 40 40 512
        ...
        self.block5_continue = nn.Sequential(OrderedDict([
            ('block5_conv4', nn.Conv2d(512, 512, kernel_size=1)),
            ('block5_relu', nn.ReLU(inplace=True))
        ]))
        ...
        self.up2 = nn.Sequential(OrderedDict([
            ('up2_conv1', nn.Conv2d(512, 512, kernel_size=1)),
        ...
        self.up3 = nn.Sequential(OrderedDict([
            ('up3_conv1', nn.Conv2d(128, 128, kernel_size=1)),
    ...
    def forward(self, x):
        ...
        up = self.up1(x5)
        up = torch.cat([x4, up], dim=3)
        b4 = side_branch(up,384,128,2)

        up = self.up2(up)
        up = torch.cat([x3, up], dim=3)
        b3 = side_branch(up,160,160,2)

        up = self.up3_b2(up)
        up = torch.cat([x2, up], dim=3)
        b2 = side_branch(up,160,160,2)

        up = self.up4(up)
        final = torch.cat([x1, up], dim=3)
        ...
        fuse = torch.cat(
            [b2, b3, b4, relation_map1, relation_map2, relation_map3, relation_map4, relation_map5, relation_map6,
             relation_map7, relation_map8], dim=3)
        fuse = self.fuse(fuse)
