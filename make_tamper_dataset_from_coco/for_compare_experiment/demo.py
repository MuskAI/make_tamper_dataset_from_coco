...
class ToleranceMetrics:
    ...
    def t0f1(self, pred_path, gt_path, edge_mode='area'):
        ...
        try:
            ...
            if len(pred.split())==3:
                pred = pred.split()[0]
            pred = pred.resize((pred.size[0]*2, pred.size[1]*2))
        ...
        for i in range(2):
            ...
            try:
                ...
                print((_.size[0]*2,_.size[1]*2))
                _ = _.resize((_.size[0]*2,_.size[1]*2))
                pred = np.array(_)
...
if __name__ == '__main__':
    ...
    pred = [r'C:\Users\musk\Desktop\0322_stage1&2_后缀为0306的模型,只监督条带区域,无8张图约束\coverage_test\pred_train\stage2',
            ...
            r'G:\论文准备\mask-columbia']
    gt = [r'G:\3月最新数据\public_dataset\coverage\gt',
          r'G:\3月最新数据\public_dataset\casia\gt',
          r'D:\lab\tamper_detection\public_data\coverage\MY_COVERAGE_DATA\gt',
          r'D:\lab\tamper_detection\public_data\Columbia\Columbia Uncompressed Image Splicing Detection\4cam_splc.tar\dou_gt500']
    tm = ToleranceMetrics()
    ...
    for idx, item in enumerate(tqdm(pred_list)):
        ...
        try:
            ...
            # result = tm.tx_f1_precision_recall(pred_path=_pred_path, gt_path=_gt_path,tolerance=4,edge_mode='edge')
        except Exception as e:
            print(e)
        f1_avg += result['f1']
